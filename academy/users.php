...
<div class="modal fade modal-light" id="m_modal_user" tabindex="-1" role="dialog" aria-labelledby="tournamentModalTitle" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered" role="document" style="min-width: 50%;">
        <div class="modal-content">
					<form class="m-form m-form--label-align-left- m-form--state-" id="m_form_user" novalidate="novalidate" method="post" action="add.php">
					<!-- 	<input type="hidden" name="field" class="form-control m-input" placeholder="" value="tournaments">
									...
									<div class="m-portlet__body">
										...
										<div class="form-group m-form__group row">
											<div class="col-lg-12 m-form__group-sub">
												...
												<select name="country" id="u" class="form-control m-input" required>
						...
						<div class="modal-footer">
							...
							<!-- <button class="btn m-btn btn-danger text-white action_tournament" type="button" data-action="delete" data-target=""><i class="fa fa-trash text-white"></i> Delete</button> -->
							<button class="btn btn-warning" id="m_user_submit"><i class="fa fa-save fa-fw"></i>Save</button>
						</div>
					</form>
        </div>
      </div>
...
<a href="addPlayers.php" class="btn btn-primary m-btn m-btn--custom m-btn--icon m-btn--air">
...
$result = queryDB("SELECT users.id as id, fullname, profession, country, email, verified, phone,approved FROM users left join players_map on users.id = players_map.user_id WHERE players_map.academy_id = '".$_SESSION['user']['id']."' and profession = 'player' ORDER BY createdAt DESC");
...
<script type="text/javascript">
  ...
  document.getElementById("u").innerHTML += '<option id="' + data[i].state_abb + '" value="' + data[i].state_abb + '">' + data[i].state_name + '</option>';
}
                }
            });
                                });
   ...
   <script type="text/javascript">
                   ...
                   $("#m_user_submit").click(function (l) {\
                   	alert('aya');
        l.preventDefault();
        var t = $(this),
          r = $(this).closest("form");
